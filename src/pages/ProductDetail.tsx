    ...
    >
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        <div className="lg:col-span-7 flex flex-col gap-10">
          <div className="aspect-[4/5] bg-surface rounded-[60px] overflow-hidden group relative shadow-2xl border border-outline/30">
            ...
            />
            <div className="absolute top-10 left-10 bg-primary text-white border border-white/20 px-6 py-2.5 rounded-full font-sans font-bold text-[10px] uppercase tracking-[0.3em] shadow-2xl">
              Bestseller
            </div>
          </div>
          <div className="grid grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className={`aspect-square bg-surface rounded-[24px] overflow-hidden cursor-pointer transition-all duration-300 border border-outline/30 shadow-sm ${i === 0 ? "ring-2 ring-primary ring-offset-2 scale-105" : "opacity-60 hover:opacity-100"}`}>
        ...
        <div className="lg:col-span-5 flex flex-col gap-12 sticky top-32 h-fit">
          <div>
            <span className="font-sans font-bold text-[10px] text-primary uppercase tracking-[0.4em] mb-4 block text-left">The Signature</span>
            <h1 className="font-serif italic text-6xl text-on-surface mb-4 leading-[0.9] tracking-tighter text-left uppercase">Silk Veil.</h1>
            <div className="flex items-center gap-6 mb-6">
              <div className="flex text-secondary items-center gap-1">
                {[...Array(5)].map((_, i) => <Star key={i} size={12} fill={i < 4 ? "currentColor" : "none"} stroke={i < 4 ? "none" : "currentColor"} />)}
              </div>
              <span className="font-sans font-bold text-[10px] text-on-surface-variant/40 uppercase tracking-[0.3em] leading-none text-left">
                (128 Rituals)
              </span>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end mb-4">
                <span className="font-sans font-bold text-[10px] uppercase tracking-[0.3em] text-on-surface">
                  Selecting Tones: <span className="text-on-surface-variant/60 font-medium">Light 02</span>
                </span>
              ...
              <div className="flex flex-wrap gap-4">
                {SHADES.map((shade) => (
                  <button
                    ...
                    title={shade.name}
                  >
                    <div className="w-full h-full rounded-full border border-black/5 shadow-inner" style={{ backgroundColor: shade.color }}></div>
                  </button>
                ))}
              </div>
            </div>
      ...
      <section className="mt-40 py-24 px-12 bg-surface-bright rounded-[80px] border border-outline/30 shadow-2xl relative overflow-hidden">
        ...
        <div className="max-w-[800px] mx-auto text-center mb-20 relative z-10">
          ...
          <h2 className="font-serif italic text-6xl text-on-surface mb-8 tracking-tighter leading-none">Mapping your Routine.</h2>
          <p className="font-serif italic text-xl text-on-surface-variant leading-relaxed">
            Based on your biological analysis, we've identified <strong className="text-primary italic">Neutral Tones</strong> as your baseline. These curations maintain your skin's natural homeostasis.
          </p>
        </div>
            ...
            <div key={item.name} className="group cursor-pointer">
              <div className="aspect-[3/4] rounded-[40px] bg-surface mb-10 overflow-hidden relative shadow-lg border border-white/40">
                ...
                <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                   <div className="bg-white/90 backdrop-blur-md px-8 py-3 rounded-xl font-sans font-bold text-[9px] uppercase tracking-widest text-primary">Explore</div>
                </div>
              </div>
              <h3 className="font-serif italic text-2xl text-on-surface mb-3 group-hover:text-primary transition-colors uppercase leading-none">{item.name}</h3>
